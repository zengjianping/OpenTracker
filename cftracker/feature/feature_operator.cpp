...
FEAT_DATA do_dft(const FEAT_DATA &xlw) {
    FEAT_DATA xlf;
...
FEAT_DATA do_windows(const FEAT_DATA &xl, std::vector<cv::Mat> &cos_win) {
    FEAT_DATA xlw;
...
void FilterSymmetrize(FEAT_DATA &hf) {
...
std::vector<cv::Mat> init_projection_matrix(const FEAT_DATA &init_sample,
...
// Do projection row std::vector x_mat[i] * projection_matrix[i]
FEAT_DATA FeatureProjection(const FEAT_DATA &x, const std::vector<cv::Mat> &projection_matrix) {
    FEAT_DATA result;
    for (size_t i = 0; i < x.size(); i++) {// for each feature
        ...
        for (size_t j = 0; j < x[i].size(); j++) {// for each channel of original feature
            ...
            cv::Mat t = x[i][j].t();
            x_mat.push_back(cv::Mat(1, x[i][j].size().area(), CV_32FC2, t.data)); // vectorize each channel map to row std::vector
        }
...
FEAT_DATA FeatureProjectionMultScale(const FEAT_DATA &x, const std::vector<cv::Mat> &projection_matrix) {
    FEAT_DATA result;
...
float FeatureComputeInnerProduct(const FEAT_DATA &feat1, const FEAT_DATA &feat2) {
...
float FeatureComputeEnergy(const FEAT_DATA &feat) {
    float res = 0;
    if (feat.empty())
        return res;
...
FEAT_DATA FeautreComputePower2(const FEAT_DATA &feats) {
    FEAT_DATA result;
    if (feats.empty()) {
        return feats;
    }
...
// vectorize features
std::vector<cv::Mat> FeatureVectorization(const FEAT_DATA &x) {
    if (x.empty())
...
FEAT_DATA FeatureVectorMultiply(const FEAT_DATA &x, const std::vector<cv::Mat> &y,
                                const bool _conj) {
    if (x.size() != y.size())
...
// features operation 
FEAT_DATA FeatureDotMultiply(const FEAT_DATA &a, const FEAT_DATA &b) {
    FEAT_DATA res;
    if (a.size() != b.size())
...
FEAT_DATA FeatureDotDivide(const FEAT_DATA &a, const FEAT_DATA &b) {
    FEAT_DATA res;
    if (a.size() != b.size())
        assert(0 && "Unmatched feature size!");
