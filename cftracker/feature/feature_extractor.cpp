...
FEAT_DATA FeatureExtractor::extractor(const cv::Mat image, const cv::Point2f pos, const std::vector<float> scales,
    ...
    if (params.useCnFeature && is_color_image) {
        cn_feat_maps = get_cn_features(img_samples[cn_feat_ind]);
...
cv::Mat FeatureExtractor::sample_patch(const cv::Mat im, const cv::Point2f posf,
        cv::Size2f sample_sz, cv::Size2f input_sz) {
            ...
            for (int j = 0; j < featuresMap.cols; j++) {
                for (int k = 0; k < Layers; k++) {
                    ...
                    featuresMap.at<cv::Vec<float, Layers>>(i, j)[k] =
                        (iImage.at<cv::Vec<float, Layers>>(ii, jj)[k] - iImage.at<cv::Vec<float, Layers>>(ii + cell, jj)[k] 
                         - iImage.at<cv::Vec<float, Layers>>(ii, jj + cell)[k] + iImage.at<cv::Vec<float, Layers>>(ii + cell, jj + cell)[k]) 
                         / (region_area * maxval);
                }
            }
        }
...
#ifdef USE_CAFFE
FEAT_DATA FeatureExtractor::get_cnn_layers(std::vector<cv::Mat> im, const cv::Mat &deep_mean_mat) {
...
*/
    ...
    for (size_t idx = 0; idx < cnn_features_.fparams.output_layer.size(); ++idx) {
        ...
        std::vector<cv::Mat> merge_feature;
...
void FeatureExtractor::cnn_feature_normalization(FEAT_DATA &cnn_feat_maps) {
    //debug("cnn_feat_maps: %lu", cnn_feat_maps.size());
    for (size_t i = 0; i < cnn_feat_maps.size(); i++) {// for each layer = 2
        std::vector<cv::Mat> temp = cnn_feat_maps[i];
