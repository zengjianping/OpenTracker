...
namespace cftracker
{
void SampleUpdate::init(const std::vector<cv::Size> &filter, const std::vector<int> &feature_dim,
        ...
        for (size_t j = 0; j < (size_t)feature_dim.size(); j++) {// for each feature
            ...
            for (size_t i = 0; i < (size_t)feature_dim[j]; i++) {// for each dimension of the feature
                temp_single_feat.push_back(cv::Mat::zeros(
                    cv::Size((filter[j].height + 1) / 2, filter[j].width), CV_32FC2));
            }
...
cv::Mat SampleUpdate::find_gram_vector(const FEAT_DATA &new_train_sample) {
...
FEAT_DATA SampleUpdate::merge_samples(const FEAT_DATA &sample1, const FEAT_DATA &sample2,
        const float w1, const float w2, const std::string sample_merge_type) {
    float alpha1 = w1 / (w1 + w2);
    float alpha2 = 1 - alpha1;
    FEAT_DATA merged_sample = sample1;
...
void SampleUpdate::update_sample_space_model(const FEAT_DATA &new_train_sample) {
    ...
    if (num_training_samples_ == nSamples_) {// if memory is full
        ...
        min_sample_weight_ = *pos;
        min_sample_id = pos - prior_weights_.begin();
        ...
        if (min_sample_weight_ < minmum_sample_weight_) {
            ...
            if (new_sample_min_dist < existing_samples_min_dist) {
                ...
                FEAT_DATA existing_sample_to_merge = samples_f_[merged_sample_id_];

                // Merge the new_train_sample with existing sample
                merged_sample_ = merge_samples(existing_sample_to_merge, new_train_sample,
                    prior_weights_[merged_sample_id_], learning_rate_, std::string("merge"));
                ...
                merged_sample_ = merge_samples(samples_f_[closest_exist_sample_pair.x],
                    samples_f_[closest_exist_sample_pair.y], prior_weights_[closest_exist_sample_pair.x],
					prior_weights_[closest_exist_sample_pair.y], std::string("merge"));
...
void SampleUpdate::update_distance_matrix(cv::Mat &gram_vector, float new_sample_norm, int id1, int id2, float w1, float w2)
{
    ...
    if (id2 < 0) {
        ...
        else { // The new sample is merge with an existing sample
    ...
    else
    {
        ...
        gram_matrix_.at<COMPLEX>(id1, id1) = COMPLEX(std::pow(alpha1, 2) * norm_id1[0] +
            std::pow(alpha2, 2) * norm_id2[0] + 2 * alpha1 * alpha2 * ip_id1_id2[0], 0);
        gram_vector.at<COMPLEX>(id1) = COMPLEX(alpha1 * gram_vector.at<COMPLEX>(id1, 0)[0] +
            alpha2 * gram_vector.at<COMPLEX>(id2, 0)[0], 0);
