#ifndef CFTRACKER_DEBUG_HPP
#define CFTRACKER_DEBUG_HPP
...
inline void showmat1ch(cv::Mat mat, int type) {
    ...
    for (int j = 0; j < mat.cols; j++) {
        if (type == 1) { // last row
            printf("%d ", mat.at<int>(mat.rows - 1, j));
        }
        else if (type == 2) { // last row
            printf("%f ", mat.at<float>(mat.rows - 1, j));
        }
        else if (type == 3) { // last row
            printf("%lf ", mat.at<double>(mat.rows - 1, j));
        }
    }
    ...
    for (int i = 0; i < mat.rows; i++) {
        ...
        else if (type == 2) { // first col
            printf("%f ", mat.at<float>(i, 0));
        }
        else if (type == 3) { // first col
    ...
    for (int i = 0; i < mat.rows; i++) {
        if (type == 1) { // last col
            printf("%d ", mat.at<int>(i, mat.cols - 1));
        }
        else if (type == 2) { // last col
            printf("%f ", mat.at<float>(i, mat.cols - 1));
        }
        else if (type == 3) { // last col
            printf("%lf ", mat.at<double>(i, mat.cols - 1));
        }
    }
    printf("\nEnd of 1 channel mat\n");
}
...
inline void showmat2ch(cv::Mat mat, int type) {
    ...
    for (int k = 0; k < mat.channels(); k++) {
        for (int j = 0; j < mat.cols; j++) {
            if (type == 3) { // last row
                printf("%lf ", mat.at<cv::Vec2d>(mat.rows - 1, j)[k]);
            }
        }
        printf("\n\n");
    }
    ...
    for (int k = 0; k < mat.channels(); k++) {
        for (int i = 0; i < mat.rows; i++) {
            if (type == 3) { // last col
                printf("%lf ", mat.at<cv::Vec2d>(i, mat.cols - 1)[k]);
            }
        }
        printf("\n\n");
    }
...
inline void showmat3ch(cv::Mat mat, int type) {
    ...
    for (int j = 0; j < mat.cols; j += 1) {
        if (type == 0) { // 2: means Red, 0 means first row
