...
#include "cftracker/trackers/eco/interpolator.hpp"
#include "cftracker/trackers/eco/regularization_filter.hpp"
#include "cftracker/trackers/eco/optimize_scores.hpp"
...
void EcoTracker::init(const cv::Mat& im, const cv::Rect2f& rect) {
    ...
    if (params_.use_scale_filter) {// fDSST
        ...
        scale_params.scale_model_sz.height = std::max((int)std::floor(rect.height * scale_params.scale_model_factor), 8);
        scale_params.scale_model_sz.width = std::max((int)std::floor(rect.width * scale_params.scale_model_factor), 8);
...
bool EcoTracker::update(const cv::Mat& frame, cv::Rect2f& roi) {
    ...
    // 2:  project sample
    FEAT_DATA xt_proj = FeatureProjectionMultScale(xt, projection_matrix_);
    ...
    FEAT_DATA xtf_proj = do_dft(xt_proj);
...
void EcoTracker::init_parameters(const EcoParameters &parameters) {
    ...
    params_.scale_params = parameters.scale_params;
...
void EcoTracker::init_features() {
...
#else
    params_.feature_params.useDeepFeature = false;
#endif
    ...
    if (feature_params.useHogFeature) {// just HOG feature;
        ...
        feature_params.hog_features.data_sz_block0 = cv::Size(
            feature_params.hog_features.img_sample_sz.width / feature_params.hog_features.fparams.cell_size,
            feature_params.hog_features.img_sample_sz.height / feature_params.hog_features.fparams.cell_size);
    ...
    if (feature_params.useCnFeature && is_color_image_) {
        ...
        feature_params.cn_features.data_sz_block0 = cv::Size(
            feature_params.cn_features.img_sample_sz.width / feature_params.cn_features.fparams.cell_size,
            feature_params.cn_features.img_sample_sz.height / feature_params.cn_features.fparams.cell_size);
        ...
        std::string s;
        std::string path = feature_params.cn_features.fparams.tablename;
    ...
    if (feature_params.useIcFeature && !is_color_image_) {
        ...
        feature_params.ic_features.data_sz_block0 = cv::Size(
            feature_params.ic_features.img_sample_sz.width / feature_params.ic_features.fparams.cell_size,
            feature_params.ic_features.img_sample_sz.height / feature_params.ic_features.fparams.cell_size);

        std::string s;
        std::string path = feature_params.ic_features.fparams.tablename;
        ...
        for (size_t i = 0; i < rows; i++) {
            ...
            getline(*read, s);
            feature_params.ic_features.fparams.table[i][cols - 1] = atof(s.c_str());
        }
    }
...
FEAT_DATA EcoTracker::interpolate_dft(const FEAT_DATA &xlf, std::vector<cv::Mat> &interp1_fs, std::vector<cv::Mat> &interp2_fs) {
    FEAT_DATA result;
    for (size_t i = 0; i < xlf.size(); i++) {
...
FEAT_DATA EcoTracker::compact_fourier_coeff(const FEAT_DATA &xf)
{
    FEAT_DATA result;
...
FEAT_DATA EcoTracker::full_fourier_coeff(const FEAT_DATA &xf) {
    FEAT_DATA res;
    for (size_t i = 0; i < xf.size(); i++) {// for each feature
        std::vector<cv::Mat> tmp;
        for (size_t j = 0; j < xf[i].size(); j++) {// for each dimension
...
std::vector<cv::Mat> EcoTracker::project_mat_energy(std::vector<cv::Mat> proj, std::vector<cv::Mat> yf) {
...
FEAT_DATA EcoTracker::shift_sample(FEAT_DATA &xf, cv::Point2f shift, std::vector<cv::Mat> kx, std::vector<cv::Mat> ky) {
    FEAT_DATA res;
